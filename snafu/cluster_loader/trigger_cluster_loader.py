...
class _trigger_cluster_loader:
    ...
    def __init__(
        self, logger, cluster_name, result_dir, user, uuid, sample, path_binary, test_name, console_cl_output
    ):
        self.logger = logger
    ...
    def emit_actions(self):
        ...
        try:
            ...
            exit(1)
        self.logger.info("completed sample {} , results in {}".format(self.sample, execution_output_file))
        file_stdout.close()
        ...
        self.logger.info("cl output is {}".format(cl_output_json))
        ...
        if config_file_location == "":
            ...
            config_file_location = (
                list(filter(pattern.match, output_file_content))[0]
                .strip()
                .split("INFO: Using config ", 1)[1]
                .replace('"', "")
            )
        self.logger.info("config file location is {}".format(config_file_location))
        with open(config_file_location, "r") as stream:
            try:
                config_dict = yaml.safe_load(stream)
            except yaml.YAMLError as exc:
                print(exc)
        self.logger.info("configuration is {}".format(config_dict))
        output_template = {}
