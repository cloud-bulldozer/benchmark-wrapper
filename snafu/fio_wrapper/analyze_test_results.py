...
for hit in hit_generator:
        ...
        try:
            ...
            if prev_job_options == None:
                raise FioStatException('did not have job options while parsing: %s' % str(src))
            rw = prev_job_options['rw']

        uuid_found = src['uuid']
        if uuid_found != uuid:
            #raise FioStatException('elastic search returned result with uuid %s not %s' % 
                    #(uuid_found, uuid))
            wrong_uuid += 1
            continue
...
print('  op-type,      block-size,  sample,      pod,           process,        iops, bw (KiB/s), elapsed-time (sec), %deviation')
try:
  for optype in sorted(optype_dict.keys()):
    ...
    for bs in optype_blksizes.keys():
        ...
        if len(sample_list_keys) < max_sample:
            raise FioStatException('op-type %s blksize %s does not have %d samples, has only %d samples' % 
        ...
        for s in sorted(sample_list_keys):
            ...
            if pods_per_run == 0:
                ...
                print('WARNING: only %d pods found in optype %s bs %s sample %d, expected %d' % 
            ...
            for p in sorted(sample_pods_keys):
                ...
                for k, job in enumerate(jobs_in_pod):
                    ...
                    if elapsed > max_elapsed_sec: max_elapsed_sec = elapsed
                    iops_in_sample.append(iops)
                    print('%10s, %5d, %14d, %14s, %16d, %9.1f, %8.3f, %8.1f' % 
            ...
            print('%10s, %5d, %14d,     add-pods,      add-processes, %9.1f, %8.3f, %8.1f, %8.3f' % 
                (optype, bs, s, total_iops_in_sample, total_MBps_in_sample, max_elapsed_sec, iops_pct_dev_in_sample))
            print('')
        ...
        print('%10s, %5d, across-samples,     add-pods,      add-processes, %9.1f, %8.3f, %8.1f, %8.3f' % 
