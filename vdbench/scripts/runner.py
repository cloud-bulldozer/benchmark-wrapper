...
base_dir = '/opt/snafu/vdbench_wrapper/vdbench'
...
def build_cmd_parameters():
    ...
    parser.add_option(
        ...
        metavar='FILE',
        default='RBD-Raw',
        help='the name of the test to run (relative to installation path)')
...
def run_vdbench(cfg, out, log):
    print('running the test : {}'.format(cfg))
    cmd = base_dir + '/bin/vdbench'
    out_path = base_dir + '/outputs/' + out
    logfile = base_dir + '/logs/' + log + '.log'
    os.system("{} -f {} -o {} > {}".format(cmd, cfg, out_path, logfile))
...
if __name__ == "__main__":
    ...
    if OPTIONS.type2run.upper() == 'B':
        ...
        for dev in block_vols():
            dev_size = os.popen('lsblk {} 2>/dev/null | grep disk'.format(
                dev)).read().strip().split()[3]
    ...
    if os.path.isdir((test2run)) is True:
        ...
        for test in os.popen('ls {}'.format(test2run)).read().split('\n'):
            if test is not '':
                t = test.split('/')[-1]
                run_vdbench('{}/{}'.format(test2run, test), t, t)
    elif os.path.isfile(test2run) is True:
        t = test2run.split('/')[-1]
        run_vdbench(test2run, test), t, t)
